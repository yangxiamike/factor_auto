...
def compute_candidate_metrics_from_matrix(
    *,
    ...
    for horizon_index, horizon in enumerate(config.horizons):
        ...
        directional_ic_mean = direction_sign * ic_mean if np.isfinite(ic_mean) else np.nan
        directional_rankic_mean = direction_sign * rankic_mean if np.isfinite(rankic_mean) else np.nan
        valid_ic = ic[np.isfinite(ic)]
        valid_rankic = rankic[np.isfinite(rankic)]
        ic_positive_ratio = float(np.mean(valid_ic > 0.0)) if valid_ic.size else np.nan
        rankic_positive_ratio = float(np.mean(valid_rankic > 0.0)) if valid_rankic.size else np.nan
        directional_ic_positive_ratio = (
            float(np.mean(valid_ic * direction_sign > 0.0)) if valid_ic.size else np.nan
        )
        directional_rankic_positive_ratio = (
            float(np.mean(valid_rankic * direction_sign > 0.0)) if valid_rankic.size else np.nan
        )
        ...
        directional_monotonicity = (
            direction_sign * monotonicity_mean if np.isfinite(monotonicity_mean) else np.nan
        )
