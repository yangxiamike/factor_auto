...
改动项目代码前，先阅读 `docs/architecture/factor-autoresearch-code-style.md`。适用范围包括 `factor_autoresearch/**/*.py`、`scripts/**/*.py`、`tests/**/*.py` 等所有 Python 代码。

- 模块开头写清职责，说明负责什么、不负责什么。
- 复杂模块用 `# ============== 分区名称 ==============` 做分区。
- 新增函数、类、公共方法使用短中文 docstring。
- 风格整理不改变公开函数签名和计算行为。
...
- 普通 Python 改动：至少运行相关 `pytest` 和 `ruff check`。
- 改动 `factor_autoresearch/compute_v1/**`、`factor_autoresearch/evaluate.py` 或 compute v1 测试：运行 `python scripts/run_compute_v1_guardrails.py`。
- 改动 compute engine 核心计算、预处理、指标或并发路径：补跑 compute v1 定向测试。
